...
describe "Page", :shared => true do
  ...
  describe "with metadata" do
    ...
    it "should not reparse the body" do
      @article.should_receive(:markup).once.and_return("")
      @article.body
      @article.body
    end
    ...
    it "should retrieve etag from metadata" do
      @article.etag.should == @etag
    end
  ...
  describe "without metadata" do
    ...
    it "should default the etag based on a hash of the body content" do
      require 'digest/sha1'
      Digest::SHA1.stub(:hexdigest).and_return('body_hash')
      @article.etag.should eql('body_hash')
    end
